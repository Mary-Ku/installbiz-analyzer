...
class ExternalApiClient:
    ...
    def __init__(self, client: AsyncClient | None = None) -> None:
        """Создает клиент, с заголовком идентификации, если он задан."""
    ...
    async def get_names(self) -> list[str]:
        """Возвращает имена еще не скачанных файлов."""
    ...
    async def _request(self, method: str, url: str, **kwargs: object) -> Response:
        """Выполняет запрос с учетом лимитов внешнего API."""
...
class DownloadService:
    ...
    def __init__(self, client: ExternalApiClient | None = None) -> None:
        """Создает сервис с клиентом внешнего API."""
    ...
    def start(self) -> bool:
        """Запускает скачивание в фоне, возвращает False, если оно уже идет."""
    ...
    async def _download_catalog(self) -> None:
        ...
        async with async_session_maker() as session:
            ...
            while names := await self._client.get_names():
                ...
                # Уже сохраненные файлы не скачиваем повторно,
    ...
    async def _download_batch(
        ...
        """Скачивает порцию файлов, возвращает числа сохраненных и отмеченных имен."""
        ...
        # Отмечаем только реально сохраненные файлы: иначе отсутствующие
...
def _batches(names: list[str], size: int) -> list[list[str]]:
    """Разбивает список имен на порции заданного размера."""
